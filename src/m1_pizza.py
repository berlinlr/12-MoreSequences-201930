...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(150, 200),  # All numbers are approximate.
                rg.Point(100, 250),
                rg.Point(50, 200),
                rg.Point(100,150)
               ]
...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    title = 'DRAW_POINTS_ON_CIRCLE, test 2:  40 red dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color= 'black'
    draw_points_on_circle(window, circle, 40, 'red')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    title = 'PIZZA test 2:  3 slices, thin (thickness=3) yellow lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 3, 'yellow', 3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    # Tests 1:
    title = ('POLYGON tests 1 :  5 segments with thin green lines;')
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 2
    polygon(window, circle, 5, 'green', 10)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(points)):
        line = rg.Line(previous_point, points[k])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
        previous_point = points[k]
    line = rg.Line(previous_point, points[0])
    line.thickness = thickness
    line.color = color
    line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 51, 25, 'purple', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    m = 0
    for k in range(len(points)):
        line = rg.Line(previous_point, points[m + hops_to_next_point])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
        window.render()
